...
class Crew extends CI_Controller {
	...
	public function index() {
		...
		$template = array(
			...
			'judulkonten' => 'Crew MKA 30 Tahun',
			'konten' => $this->load->view('crew/viewdata', '', true),
	...
	public function simpan() {
		...
		$agt = $this->input->post('agt');
		$mka = $this->input->post('mka');
	...
	// 	  $upload = $this->modal->upload_file($this->filename);
	...
	// 		$loadexcel = $excelreader->load('./assets/upload_excel'.$this->filename.'.xlsx'); // Load file yang tadi diupload ke folder excel
	...
	//   }
	  ...
	  public function import(){
		...
		$excelreader = new PHPExcel_Reader_Excel2007();
		$loadexcel = $excelreader->load('./assets/upload_excel/'.$this->filename.'.xlsx'); // Load file yang telah diupload ke folder excel
		$sheet = $loadexcel->getActiveSheet()->toArray(null, true, true ,true);
		...
		foreach($sheet as $row){
		  ...
		  if($numrow > 3){
			...
			array_push($data, array(
				'id'=>$row['B'], // Insert data nis dari kolom A di excel
				'nama'=>$row['C'], // Insert data nama dari kolom B di excel
				'jabatan'=>$row['D'], // Insert data jenis kelamin dari kolom C di excel
				'unit'=>$row['E'],
				'agt_klrg'=>$row['F'],
				'tgl_30'=>$row['G'],
				'tgl_validity'=>$row['H'] // Insert data alamat dari kolom D di excel
			));
		  }
		...
		$this->modal->insert_multiple($data);
		
		redirect("crew"); // Redirect ke halaman awal (ke controller siswa fungsi index)
	  }
